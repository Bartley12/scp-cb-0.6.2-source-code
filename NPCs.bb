...
Const NPCtype173% = 1, NPCtypeOldMan% = 2, NPCtypeGuard% = 3, NPCtypeD% = 4 ; Updated for Blitz3D 1.108 compatibility
Const NPCtype372% = 6, NPCtypeApache% = 7, NPCtypeMTF% = 8, NPCtype096 = 9 ; Updated for Blitz3D 1.108 compatibility
	...
	Select NPCtype
		Case NPCtype173
			...
			If BumpEnabled Then 		
				...
				; FastExt compatibility - FE_BUMP replacement for Blitz3D 1.108
				; Note: Actual bump mapping requires FastExt library
		...
		Case NPCtype096
			...
			temp# = (GetINIFloat("DATA\NPCs.ini", "SCP-096", "scale") / 7.5)
			ScaleEntity n\obj, temp, temp, temp	
		Case NPCtypeApache
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.2
	...
	For n.NPCs = Each NPCs
		Select n\NPCtype
				...
				Select n\State
					Case 0
						If EntityDistance(Collider, n\Collider)<8.0 Then
							...
							Animate2(n\obj, AnimTime(n\obj),569,720, 0.25)
							pvt = CreatePivot()
					...
					Case 4
						...
						If EntityVisible(Collider, n\Collider) And KillTimer => 0 Then
							...
							If dist<0.6 Then
								...
								PointEntity n\Collider, Collider
								RotateEntity n\Collider, 0, EntityYaw(n\Collider)+Sin(n\State2*2.5)*30, 0
							...
							Animate2(n\obj, AnimTime(n\obj),528,568, n\currspeed*15)
						...
						Else
							...
							Else
								Animate2(n\obj, AnimTime(n\obj),200,220, 0.1)
					...
					Default
						...
						If n\State=1 Then
							Animate2(n\obj, AnimTime(n\obj),569,785, 0.3, False)
							If AnimTime(n\obj)>784 Then n\State = 2
						ElseIf n\State=2
							Animate2(n\obj, AnimTime(n\obj),5,100, 0.15, False)
							If AnimTime(n\obj)>98 And AnimTime(n\obj)=<100 Then n\State = 3
						ElseIf n\State=3
							n\State2 = n\State2+FPSfactor
							If n\State2 > 70*25 Then 
								Animate2(n\obj, AnimTime(n\obj),200,228, 0.1, False)
								If AnimTime(n\obj)>227.5 Then 
									n\State = 4
									StopChannel n\SoundChn
									FreeSound n\Sound
									n\Sound = 0
								EndIf
							Else
								Animate2(n\obj, AnimTime(n\obj),200,220, 0.25)
							EndIf
							RotateEntity(n\Collider, Sin(n\State2*3)*10, EntityPitch(n\Collider)+Sin(n\State2^1.2)*8, 0, True)
						EndIf
				End Select
			...
			Case NPCtypeD 	;------------------------------------------------------------------------------------------------------------------
				...
				If n\State = 1 Then
					MoveEntity(n\Collider, 0, 0, 0.015 * FPSfactor)
					Animate2(n\obj, AnimTime(n\obj), 0, 14, 0.15)
				End If
				
				PositionEntity(n\obj, EntityX(n\Collider), EntityY(n\Collider) - 0.25, EntityZ(n\Collider))
